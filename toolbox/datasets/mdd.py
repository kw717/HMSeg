...
class MDD(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = Image.open(os.path.join(self.root, 'Images',  image_path))  # RGB 0~255
        label = Image.open(os.path.join(self.root, 'Masks',  image_path))  # 1 channel 0~11
...
if __name__ == '__main__':
    ...
    path = 'E:\sonar\sonar_seg\configs\wsdata.json'
    with open(path, 'r') as fp:
        cfg = json.load(fp)
    #cfg['root'] = '/home/dtrimina/Desktop/lxy/database/camvid'


    dataset = MDD(cfg, mode='trainval', do_aug=True)
